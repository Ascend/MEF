...
|haveged|<li>Ubuntu操作系统执行以下命令安装：<ul>```apt install haveged```</ul></li><li>openEuler操作系统执行以下命令安装：<ul>```yum install haveged```</ul></li><div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">haveged安装后，需要执行以下命令启动haveged服务：<br>```systemctl enable haveged```<br>```systemctl start haveged```<br></div></div>|
...
3. 执行以下命令，进行本地域名映射配置。
    ...
    **表 1**  domainconfig参数说明表<a id="domainconfig参数说明表"></a>
    ...
    |domain|第三方镜像仓库域名，支持长度3~63位，大小写字母、数字和符号（.-）的组合，且只能以大小写字母、数字开头和结尾，但是不能全为数字。<br><div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">第三方镜像仓库域名不能为localhost或其他等价于localhost的域名。</div></div>|
...
3. 执行以下命令，查询MEF Edge云边对接根证书备份信息。
...
3. 执行以下命令，删除未使用的MEF Edge云边对接根证书。
...
2. 执行以下步骤准备Dockerfile文件。
    ...
    3. 写入以下内容后执行<b>:wq</b>命令保存，以Ubuntu  Arm操作系统为例（以下内容仅为编写示例，请用户根据实际情况结合自身理解进行二次开发）。
        ...
        # 创建运行推理应用的用户及组，以步骤2第1点中查到的HwHiAiUser，HwDmUser，HwBaseUser的UID与GID分别为1000，1101，1102为例
        RUN umask 0022 && \
