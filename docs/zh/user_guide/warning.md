...
1. 检查MEF Edge日志目录是否有足够的存储空间。
    - 执行**df** **-lh**命令，查看各分区的占用率。当分区占用率超过80%时，会上报告警。
        ...
        显示信息如下：表示/var/alog目录占用率为92%，产生告警。
...
### 0x01000001 第三方管理平台根证书超期（严重告警）<a name="ZH-CN_TOPIC_0000001722295473"></a>
...
### 0x01000002 软件仓根证书超期（严重告警）<a name="ZH-CN_TOPIC_0000001722375505"></a>
...
### 0x01000003 镜像仓根证书超期（严重告警）<a name="ZH-CN_TOPIC_0000001674256222"></a>
