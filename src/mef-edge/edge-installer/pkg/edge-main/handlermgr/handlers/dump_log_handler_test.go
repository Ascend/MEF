...
func createDumpLogMsg() (model.Message, error) {
	var dumpLogMsg model.Message
...
func parseAndCheckArgsFailed() {
	...
	convey.Convey("parse content failed", func() {
		...
		defer p1.Reset()
		dumpLogMsg, err := createDumpLogMsg()
		if err != nil {
			panic(err)
		}
	...
	convey.Convey("invalid Module argument", func() {
		invalidDumpLogMsg, err := createDumpLogMsg()
		if err != nil {
			panic(err)
		}
		...
		err = handler.Handle(&invalidDumpLogMsg)
