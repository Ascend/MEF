// Copyright (c) Huawei Technologies Co., Ltd. 2025-2025. All rights reserved.
// MEF is licensed under Mulan PSL v2.
// You can use this software according to the terms and conditions of the Mulan PSL v2.
// You may obtain a copy of Mulan PSL v2 at:
//          http://license.coscl.org.cn/MulanPSL2
// THIS SOFTWARE IS PROVIDED ON AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND,
// EITHER EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO NON-INFRINGEMENT,
// MERCHANTABILITY OR FIT FOR A PARTICULAR PURPOSE.
// See the Mulan PSL v2 for more details.
//go:build MEFEdge_SDK

// Package tasks for testing prepare dir
package tasks

import (
	"testing"

	"github.com/agiledragon/gomonkey/v2"
	"github.com/smartystreets/goconvey/convey"

	"huawei.com/mindx/common/fileutils"
	"huawei.com/mindx/common/test"

	"edge-installer/pkg/common/constants"
)

func TestPrepareDir(t *testing.T) {
	p := gomonkey.ApplyFuncReturn(fileutils.IsExist, true).
		ApplyFuncReturn(fileutils.DeleteAllFileWithConfusion, nil).
		ApplyFuncReturn(fileutils.CreateDir, nil)
	defer p.Reset()

	convey.Convey("prepare dir success", t, func() {
		err := PrepareDir(constants.EdgeInstaller).Run()
		convey.So(err, convey.ShouldBeNil)
	})

	convey.Convey("create path failed", t, func() {
		p1 := gomonkey.ApplyFuncSeq(fileutils.CreateDir, []gomonkey.OutputCell{
			{Values: gomonkey.Params{test.ErrTest}},

			{Values: gomonkey.Params{nil}},
			{Values: gomonkey.Params{test.ErrTest}},

			{Values: gomonkey.Params{nil}, Times: 2},
			{Values: gomonkey.Params{test.ErrTest}},
		})
		defer p1.Reset()
		const testCaseNum = 3
		for i := 0; i < testCaseNum; i++ {
			err := PrepareDir(constants.EdgeInstaller).Run()
			convey.So(err, convey.ShouldResemble, test.ErrTest)
		}
	})
}
