...
function execute_alarm_manager_ut() {
  go mod tidy
  echo "alarm test dir: ${TOP_DIR}/pkg/..."
  if ! (go test -tags=TESTCODE -gcflags=all=-l -v -mod=mod -coverprofile cov.out "${TOP_DIR}/pkg/..." >"./$file_input"); then
    echo '****** alarm-manager go test cases error! ******'
    cat "$file_input"
    exit 1
  else
...
file_input='testAlarmManager.txt'
file_detail_output='api.html'
DB_PATH="/home/data/config/"

echo "************************************* Start Alarm-Manager LLT Test *************************************"
...
rm -rf "${DB_PATH}"
echo "************************************* End Alarm-Manager LLT Test *************************************"
