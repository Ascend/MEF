...
func CheckOriginPath(filePath string) (string, error) {
	...
	if strings.Contains(filePath, parentDirFlag) {
		return "", errors.New("the input path contains unsupported flag for parent directory")
	}
	realPath, err := CheckPath(filePath)
	if err != nil {
		return "", err
	}
	if realPath != filepath.Clean(filePath) {
		return "", errors.New("the input path not equal the realpath")
	}
	return realPath, nil
}

// CheckPath  validate given path and return resolved absolute path
// support the relative path, for example:  ../ in path will be ok
...
// CreateDir is used to make sure a path exists by creating it if not
func CreateDir(tgtPath string, mode os.FileMode) error {
	...
	if _, err := CheckOriginPath(tgtPath); err != nil {
		return err
	}

	return os.MkdirAll(tgtPath, mode)
}
