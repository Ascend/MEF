...
function build_pause_image() {
    isula-build ctr-img build --build-arg ARCH=${ARCH} --build-arg LD_PATH=${LD_PATH} -f Dockerfile --build-static='build-time=2022-12-23 11:34:30' -o docker-archive:${CUR_DIR}/pause.tar.gz:k8s.gcr.io/pause:latest
    if [[ $? -ne 0 ]]; then
        return 1
    fi
