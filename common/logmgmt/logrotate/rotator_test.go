...
func testCheckLogsStep1(rotator *LogRotator, logFileName, backupDir string, currentTime time.Time) {
	...
	data, err := fillData(logFileName, size1*common.MB+1)
	convey.So(err, convey.ShouldBeNil)
	mockTime(currentTime, func() {
		rotator.checkLogs()
	})
...
func createBackupFile(logBaseName, backupDir string) (string, error) {
	...
	if f, err := os.OpenFile(fileName, os.O_WRONLY|os.O_CREATE|os.O_CREATE, defaultLogPermission); err != nil {
		return "", err
...
func fillData(filename string, length int) ([]byte, error) {
	if err := os.MkdirAll(filepath.Dir(filename), logDirPermission); err != nil && os.IsExist(err) {
		return nil, err
	}
