...
func (nc *StudentChecker) init() {
	nc.modelChecker.Checker = GetAndChecker(
		GetListChecker("class", GetClassChecker(""), minListLen, maxListLen, true),
		GetRegChecker("name", "^[a-z]{1,10}$", true),
		GetIntChecker("age", minAge, maxAge, true),
		GetStringChoiceChecker("sex", []string{"man", "woman"}, true),
		GetListChecker("like", GetIntChecker("", minLike, maxLike, true),
			minListLen,
			maxListLen,
			true,
		),
...
func (cc *ClassChecker) init() {
	cc.modelChecker.Checker = GetAndChecker(
		...
		GetFloatChecker("income", 0.0, maxIncome, true),
		GetBoolChecker("topClass", true),
	)
}
