...
func (pc *PwdChecker) Check(data interface{}) checker.CheckResult {
...
func (pc *PwdChecker) isPwdValid(name, value string) checker.CheckResult {
	if len(value) < pc.minLen || len(value) > pc.maxLen {
		...
		return checker.NewFailedResult(fmt.Sprintf("the length is not in range [%d, %d]", pc.minLen, pc.maxLen))
	}

	if err := CheckPassWord(name, &value); err != nil {
		hwlog.RunLog.Errorf("Pwd checker Check [%s] failed, error: %v", pc.fieldPwd, err)
		return checker.NewFailedResult(err.Error())
	}
