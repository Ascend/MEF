...
function mv_file() {
    ...
    cd "${TOP_DIR}/savedir"
    fakeroot tar -zcf "Ascend-mefcenter_${build_version}_linux-${arch_type}.tar.gz" ./*

    mkdir -p "${TOP_DIR}/output"

    mv "${TOP_DIR}/savedir/Ascend-mefcenter_${build_version}_linux-${arch_type}.tar.gz" "${TOP_DIR}/output/"
    ...
    cd "${TOP_DIR}/output"
    zip -r "Ascend-mefcenter_${build_version}_linux-${arch_type}.zip" .
