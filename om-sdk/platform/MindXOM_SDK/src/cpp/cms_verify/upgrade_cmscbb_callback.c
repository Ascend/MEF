 ...
 * Description  : memory release
 * Params
 *   [IN] ptr: memory address
 ...
 * Params
 *   [IN] s1: memory address 1
 *   [IN] s2: memory address 2
 *   [IN] n: memory size to compare
 * Return Value : CVB_INT
 ********************************************************************************/
 ...
 * Description  : compare the first n bytes of the string
 * Params
 *   [IN] s1: string to be compared
 *   [IN] s2: string to be compared
 *   [IN] n: length of the string to be compared
 * Return Value : CVB_INT
 ********************************************************************************/
...
* Description  : string lookup function
* Params
*   [IN] haystack: searched string
*   [IN] needle: target string
* Return Value : const CVB_CHAR*
*******************************************************************************/
...
* Description  : character lookup function
* Params
*   [IN] s: searched string
*   [IN] c: target character
* Return Value : CVB_CHAR*
*******************************************************************************/
...
* Description  : Obtains the character string length.
* Params
*   [IN] s: target string
* Return Value : CVB_UINT32 the length of string
*******************************************************************************/
...
*   [IN] s1: string to be compared
*   [IN] s2: string to be compared
* Return Value : CVB_INT comparison results
*******************************************************************************/
CVB_INT CmscbbStrCmp(const CVB_CHAR* s1, const CVB_CHAR* s2)
{
    ...
    return strncmp((const char*)s1, (const char*)s2, len1);
}
...
* Params
*   [IN] ptr: data address
*   [IN] size: data read size
*   [IN] hFile: file handle
* Return Value : CVB_SIZE_T actual data read size
*******************************************************************************/
...
* Params
*   [IN] hFile: file handle
* Return Value : CVB_UINT64 file size
*******************************************************************************/
...
* Description  : log callback interface
* Params
*   [IN] log_level: log level
*   [IN] filename: file name, default cmscbb_log.log
*   [IN] line: log line
*   [IN] function: log function, when the macro CMSCBB_FUNCTION_NAME is on, is cmscbb_function.name
*   [IN] rc: error code
*   [IN] log: long info
* Return Value : CVB_VOID
* Detail Description : Each product can adapt to its own log printing mode.
*******************************************************************************/
...
#define DIG_EST_SIZE_SHA256 32
#define DIG_EST_SIZE_SHA384 48
#define DIG_EST_SIZE_SHA512 64

typedef enum HashAlgoSt {
    HSAG_UNKNOW = 0,
    HSAG_SHA256 = 3,
    HSAG_SHA384 = 4,
    HSAG_SHA512 = 5,
...
CMSCBB_ERROR_CODE CmscbbMdCreateCtx(CMSCBB_CRYPTO_MD_CTX* mdCtx)
{
    ...
    CMSCBB_ERROR_CODE ret = CmscbbMalloc((void**)&md, sizeof(CryptoMd));
    if (ret != CVB_SUCCESS) {
        return ret;
    }
...
CMSCBB_ERROR_CODE CmscbbCryptoVerifyCreateCtx(CMSCBB_CRYPTO_VRF_CTX* ctx)
{
    ...
    if (memset_s((void*)vrf, sizeof(CryptoVrf), 0, sizeof(CryptoVrf)) != 0) {
        OM_LOG_ERROR("memset_s fail.");
        CmscbbFree((void*)vrf);
        return CMSCBB_ERR_SYS_MEM_ALLOC;
    }
...
static OSSL_PARAM_BLD *CvbObtainRsaBld(CmscbbBigInt* nva, CmscbbBigInt* eva, BIGNUM **resN, BIGNUM **resE)
{
    ...
    if (bld == NULL) {
        OM_LOG_ERROR("OSSL_PARAM_BLD_new failed.");
        return NULL;
    }
    if (n != NULL && e != NULL) {
        ret = OSSL_PARAM_BLD_push_BN(bld, OSSL_PKEY_PARAM_RSA_N, n);
        if (ret <= 0) {
            OM_LOG_ERROR("OSSL_PARAM_BLD_push_BN pub key n is null(%d).", ret);
            OSSL_PARAM_BLD_free(bld);
            return NULL;
        }
        ret = OSSL_PARAM_BLD_push_BN(bld, OSSL_PKEY_PARAM_RSA_E, e);
        if (ret <= 0) {
            OM_LOG_ERROR("OSSL_PARAM_BLD_push_BN pub key e is null(%d).", ret);
            OSSL_PARAM_BLD_free(bld);
            return NULL;
        }
    } else {
...
static const char *InGetEccKeyCurName(CVB_INT32 curveId)
{
    switch (curveId) {
        case CMSCBB_NIST_CURVE_P256:
            return "prime256v1"; // NID_X9_62_prime256v1
        case CMSCBB_NIST_CURVE_P384:
            return "secp384r1"; // NID_secp384r1
        case CMSCBB_NIST_CURVE_P512:
            return "secp521r1"; // NID_secp521r1
        case CMSCBB_BRAINPOOL_CURVE_P256:
            return "brainpoolP256r1"; // NID_brainpoolP256r1
        case CMSCBB_BRAINPOOL_CURVE_P384:
            return "brainpoolP384r1"; // NID_brainpoolP384r1
        case CMSCBB_BRAINPOOL_CURVE_P512:
            return "brainpoolP512r1"; // NID_brainpoolP512r1
        case CMSCBB_CURVE_SM2:
            return "SM2"; // NID_sm2
        default:
             break;
    }
...
static OSSL_PARAM_BLD *InObtainEcKeyBld(CmscbbKeyAndAlgInfo* info)
{
    CVB_INT32 ret;
    ...
    ret = OSSL_PARAM_BLD_push_utf8_string(bld, OSSL_PKEY_PARAM_GROUP_NAME, InGetEccKeyCurName(info->curveId), 0);
    if (ret <= 0) {
        OM_LOG_ERROR("OSSL_PARAM_BLD_push_utf8_string group name failed.");
        OSSL_PARAM_BLD_free(bld);
        return NULL;
    }
    ret = OSSL_PARAM_BLD_push_octet_string(bld, OSSL_PKEY_PARAM_PUB_KEY, info->eccKey->aVal, info->eccKey->uiLength);
    if (ret <= 0) {
        OM_LOG_ERROR("OSSL_PARAM_BLD_push_utf8_string pubkey failed.");
        OSSL_PARAM_BLD_free(bld);
        return NULL;
    }
    return bld;
