...
func listEdgeNodeAlarmsOrEvents(req types.ListAlarmOrEventReq, AlarmOrEvent string) common.RespMsg {
	alarms, err := AlarmDbInstance().listEdgeAlarmsOrEventsDb(req.PageNum, req.PageSize, req.NodeId, AlarmOrEvent)
	if err != nil {
		hwlog.RunLog.Errorf("failed to list edge node[%d] %s in db,err:%s", req.NodeId, AlarmOrEvent, err.Error())
...
func getMapResult(nodes []types.NodeInfo, queryIdType string, req types.ListAlarmOrEventReq) (
	...
	for _, node := range nodes {
		alarmsNode, err := AlarmDbInstance().listEdgeAlarmsOrEventsDb(req.PageNum, req.PageSize, node.ID, queryIdType)
		if err != nil {
			return nil, fmt.Errorf("faild to list alarms of node[%d] in db while list group alarms", node.ID)
		}
