...
const (
	...
	// MaximumFileMaxSize  the max value of FileMaxSize is 400 MB
	MaximumFileMaxSize = 400
...
type LogConfig struct {
	...
	CacheSize int
	// EscapeHtml true-will trans special char to unicode or just delete, false-default not to do the trans
	EscapeHtml bool
	// DisableRotationIfSwitchUser disable rotation if switch user to avoid improper file permission
...
func validateLogConfigFileMaxSize(config *LogConfig) error {
	...
	if config.FileMaxSize < 0 || config.FileMaxSize > MaximumFileMaxSize {
		return fmt.Errorf("the size of a single log file range is (0, 400] MB")
	}
...
func validateLogConfigFiled(config *LogConfig) error {
	...
	if _, err := fileutils.CheckOriginPath(config.LogFileName); err != nil && err != os.ErrNotExist {
		return fmt.Errorf("config log path is not absolute path: %v", err)
	}
	if config.BackupDirName != "" {
		if _, err := fileutils.CheckOriginPath(config.BackupDirName); err != nil && err != os.ErrNotExist {
			return fmt.Errorf("config log backup directory check error: %v", err)
		}
		if err := checkDir(config.BackupDirName); err != nil {
			return err
		}
	}
...
func changeFileMode(l *logger, event fsnotify.Event, logFileFullPath string) {
	...
	if !fileutils.IsExist(changedLogFilePath) {
		return
	}
	fPath, err := fileutils.CheckOriginPath(changedLogFilePath)
	if err != nil {
