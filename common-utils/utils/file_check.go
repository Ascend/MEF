...
// writeWhiteList some operating system has write permission of group on these file
var writeWhiteList = []string{"/usr/bin/systemctl", "/usr/sbin/xtables-nft-multi"}
...
func inWriteWhiteList(filepath string) bool {
	for _, wl := range writeWhiteList {
		if wl == filepath {
			return true
		}
	}
...
func checkOwnerAndPermission(fileInfo os.FileInfo, filePath string) error {
	...
	gWriteIndex := groupWriteIndex
	// if in white list, not check the group write permission
	if inWriteWhiteList(filePath) {
		gWriteIndex = -1
	}
	...
	for index, char := range perm {
		if (index == gWriteIndex || index == otherWriteIndex) && char == 'w' {
